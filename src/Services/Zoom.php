...
namespace Tsakib360\LaraZoom\Services;
...
class Zoom
{
    ...
    private function generateJWTKey() {
        $key = config('larazoom.zoom_api_key');
        $secret = config('larazoom.zoom_api_secret');
        $token = array(
            "iss" => $key,
    ...
    public function createMeeting($data = array())
    {
        ...
        return static::createMeetingSendRequest($createMeetingArray);
    }

    //function to send request
    protected function createMeetingSendRequest($data)
    {
        $request_url = "https://api.zoom.us/v2/users/".config('larazoom.zoom_api_email')."/meetings";

        $headers = array(
            "authorization: Bearer ".static::generateJWTKey(),
